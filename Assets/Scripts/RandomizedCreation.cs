...
public class RandomizedCreation
{
    ...
    {
        return new BeetleInfo(Random.Range(0,2), Random.Range(1,10), Random.Range(0,10), 0, createRandomIntestineCell(null));
    }
    ...
    public static ExtremityRootCell createRandomExtremityRootCell(double cellSize, Cell parent)
    {
        ...
        {
            cells[2,topheight] = createRandomIntestineCell(parent);
        } else {
    ...
    public static IntestineCell createRandomIntestineCell(Cell parent)
    {
        int thickness = UnityEngine.Random.Range(1,4);
        double cellSize = UnityEngine.Random.Range(1,10); //should also work out with very small mainIntestineCells
        int mainXSize = Random.Range(1,10);
        int mainYSize = Random.Range(1,10);
        int cellColumnAmount = 4 * thickness + 2 * mainXSize + 2 * mainYSize;

        IntestineCell intestineCell =  new IntestineCell(true, mainXSize * cellSize, mainYSize * cellSize, parent);
        ...
        {
            //build corners
            Cell[,] corner = generateCorner(thickness, cellSize, intestineCell);
            ...
            if(y) 
            {
                ...
                for(int i = 0; i < mainYSize; i++)
                {
                    for(int j = 0; j < thickness; j++)
                    {
                        if(Random.Range(0,99) < 95)
                        {
                            cells[current,j] = new MeatCell(true, cellSize, intestineCell);
                        }
                        else
                        {
                            break;
        ...
        intestineCell.Cells =  (Cell[,])cells.Clone();// not call by rference for better readability
        
        return intestineCell;
    }
    ...
    public static Cell randomizedCell(double cellSize, Cell parent)// Cant return intestine Cell
    {
        int randomSeed = Random.Range(0,999);
        
        if(randomSeed > 7){//Meat
            return new MeatCell(true, cellSize, parent);
